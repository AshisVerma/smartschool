...

{
    ...
    public function classsectionreport(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $class_id = $this->input->post('class_id');
            $section_id = $this->input->post('section_id');
            $data['class_section_list'] = $this->classsection_model->getClassSectionStudentCount($class_id, $section_id);
            //echo"<pre>";print_r($data['class_section_list']);die;
            $this->load->view('layout/header', $data);
            $this->load->view('reports/classsectionreport', $data);
            $this->load->view('layout/footer', $data);
        }
     ...
     function pertransporterReport() {
        
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/Transport');
        $this->session->set_userdata('subsub_menu', 'Reports/student_information/studenttransportreport');

        $data['title'] = 'Student Transportation Report';
        $listroute = $this->route_model->listroute();
        $data['listroute'] = $listroute;
        $this->form_validation->set_rules('listroute', $this->lang->line('route'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view("layout/header", $data);
            $this->load->view("student/studentPerTransporterReport", $data);
            $this->load->view("layout/footer", $data);
        }else{
                    

        $route_id = $this->input->post("listroute");

        $listroute = $this->route_model->listrouteByRouteId($route_id);
        $data['studentlist'] = $listroute;  
    ...
    public function studentperareareport(){
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/Transport');
        $this->session->set_userdata('subsub_menu', 'Reports/student_information/studentperareareport');

        $data['title'] = 'Student per Area Report';
        
        $listaddress = $this->route_model->listStudentAddress();
        $data['listaddress'] = $listaddress;
        $this->form_validation->set_rules('listaddress', $this->lang->line('listaddress'), 'trim|required|xss_clean');

        if ($this->form_validation->run() == FALSE) {
            $this->load->view("layout/header", $data);
            $this->load->view("student/studentPerAreaReport", $data);
            $this->load->view("layout/footer", $data);
        }else{

            $address = $this->input->post("listaddress");
...

    {
        ...
        $this->session->set_userdata('subsub_menu', 'Reports/student_information/studentpaymentreport');
            $subtotal=false;
        ...
        $this->load->view('student/studentpaymentReport', $data);
...

    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/student_information/studentdiscountreport');
            $subtotal=false;
        if (isset($_POST['search_type']) && $_POST['search_type'] != '') {
            $dates               = $this->customlib->get_betweendate($_POST['search_type']);
            $data['search_type'] = $_POST['search_type'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['results'] = $this->studentfeemaster_model->getDiscountCollectionReport($start_date, $end_date,$feetype_id, $received_by, $group);
        ...
        $this->load->view('student/studentDiscountReport', $data);
    ...
    public function printmultiple(){
        ...
        foreach ($student_array as $key => $value) {
            $studentid_arr[] = $value->student_id;
        }
        $data['students'] = $this->student_model->getstudentsById($studentid_arr, 1);
        
        $student_report = $this->load->view('student/printselected', $data, true);
        echo  $student_report;
...

    {
        ...
        if (!empty($students->data)) {
            ...
            foreach ($students->data as $student_key => $student) {
                ...
                $row[] = "<input type='checkbox' class='checkbox center-block' data-student_id='".$student->id."' name='check' id='check' value='".$student->id."'>";
...
//datatable function to check search parameter validation
    ...
    public function getStudentByvehicleId()

    {
        ...
        $student_list         = $this->student_model->getStudentBy_vehicle_id($vehicle_id);
     ...
     public function getStudentByAddress()

    {
        ...
        $student_list         = $this->student_model->getStudentBy_address($address);
    ...
    public function addroute(){
        ...
        $this->session->set_userdata('sub_menu', 'student/addroute');

        $data['title']           = 'Student add route';
        ...
        $this->load->view('student/addRoute', $data);

        $this->load->view('layout/footer', $data);

    }

    public function addStudentRoute()
        {
        ...
        if (!empty($students->data)) {
            ...
            foreach ($students->data as $student_key => $student) {
                ...
                $routebtn = "<a href='" . base_url() . "student/route/" . $student->id . "'   class='btn btn-default btn-xs'  data-toggle='tooltip' data-placement='left' title='Add Route'><i class='fa fa-bus ftlayer'></i></a>";
    ...
    public function route($id){
        ...
        $vehroute_result            = $this->vehroute_model->getRoute($id);

        $data['vehroutelist']       = $vehroute_result;

        $this->load->view('layout/header', $data);

        $this->load->view('student/studentRoute', $data);
    ...
    public function updateRoute(){
        ...
        $this->student_model->updateRoute($data);
